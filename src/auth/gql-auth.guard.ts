...
export class GqlAuthGuard extends AuthGuard('jwt') {
  ...
  getRequest(context: ExecutionContext) {
    ...
    return { headers: { authorization: gqlCtx.connectionParams?.Authorization || gqlCtx.connectionParams?.authorization } };
  }

  // Кастомизируем ошибку, если пользователь не авторизован
  handleRequest(err: any, user: any) {
    if (err || !user) {
      throw err || new UnauthorizedException('Вы должны войти в систему для выполнения этого действия');
    }
    return user;
  }
}
